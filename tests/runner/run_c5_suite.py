...
TEST_MODULE = "tests.semantic.test_c5_pack_contract"
...
def blocked_socket(*args: Any, **kwargs: Any) -> Any:
    raise RuntimeError("external network is disabled by the C5 runner")
...
def main() -> int:
    ...
    previous_adapter = os.environ.get("NOETIDE_C5_ADAPTER")
    ...
    try:
        os.environ["NOETIDE_C5_ADAPTER"] = args.adapter
        ...
        if previous_adapter is None:
            os.environ.pop("NOETIDE_C5_ADAPTER", None)
        else:
            os.environ["NOETIDE_C5_ADAPTER"] = previous_adapter
    ...
    artifact = {
        "schema_version": "noetide.c5-run-result.v1",
        "run_id": f"c5-{uuid.uuid4().hex}",
        ...
        "command": ["python", "-m", "tests.runner.run_c5_suite", *sys.argv[1:]],
    ...
    print(f"{'passed' if passed else 'failed'}: {len(rows)} C5 scenarios; artifact={output.relative_to(ROOT)}")

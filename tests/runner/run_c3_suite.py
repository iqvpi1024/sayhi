...
TEST_MODULE = "tests.semantic.test_c3_review_contract"
...
def blocked_socket(*args: Any, **kwargs: Any) -> Any:
    raise RuntimeError("external network is disabled by the C3 runner")
...
def main() -> int:
    ...
    previous_adapter = os.environ.get("NOETIDE_C3_ADAPTER")
    ...
    try:
        os.environ["NOETIDE_C3_ADAPTER"] = args.adapter
        ...
        if previous_adapter is None:
            os.environ.pop("NOETIDE_C3_ADAPTER", None)
        else:
            os.environ["NOETIDE_C3_ADAPTER"] = previous_adapter
    ...
    artifact = {
        "schema_version": "noetide.c3-run-result.v1",
        ...
        "command": ["python", "-m", "tests.runner.run_c3_suite", *sys.argv[1:]],
    ...
    print(f"{'passed' if passed else 'failed'}: {len(rows)} C3 scenarios; artifact={output.relative_to(ROOT)}")

...
TEST_MODULE = "tests.semantic.test_b2_episode_summary_contract"
...
def blocked_socket(*args: Any, **kwargs: Any) -> Any:
    raise RuntimeError("external network is disabled by the B2 runner")
...
def main() -> int:
    ...
    previous_adapter = os.environ.get("NOETIDE_B2_ADAPTER")
    ...
    try:
        os.environ["NOETIDE_B2_ADAPTER"] = args.adapter
        ...
        if previous_adapter is None:
            os.environ.pop("NOETIDE_B2_ADAPTER", None)
        else:
            os.environ["NOETIDE_B2_ADAPTER"] = previous_adapter
    ...
    artifact = {
        "schema_version": "noetide.b2-run-result.v1",
        ...
        "command": ["python", "-m", "tests.runner.run_b2_suite", *sys.argv[1:]],
    ...
    print(f"{'passed' if passed else 'failed'}: {len(rows)} B2 scenarios; artifact={output.relative_to(ROOT)}")

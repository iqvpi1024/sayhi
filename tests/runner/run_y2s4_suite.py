...
TEST_MODULE = "tests.semantic.test_y2s4_cloud_model_contract"
...
class LoopbackOnlySocket(_ORIGINAL_SOCKET):
    def connect(self, address: Any) -> None:
        if not _is_loopback_address(address):
            raise RuntimeError("external network is disabled by the Y2S4 runner")
...
def main() -> int:
    ...
    previous_adapter = os.environ.get("NOETIDE_Y2S4_ADAPTER")
    socket.socket, socket.create_connection = LoopbackOnlySocket, loopback_create_connection
    try:
        os.environ["NOETIDE_Y2S4_ADAPTER"] = args.adapter
        ...
        if previous_adapter is None:
            os.environ.pop("NOETIDE_Y2S4_ADAPTER", None)
        else:
            os.environ["NOETIDE_Y2S4_ADAPTER"] = previous_adapter
    ...
    artifact = {
        "schema_version": "noetide.y2s4-run-result.v1",
        "run_id": f"y2s4-{uuid.uuid4().hex}",
        ...
        "command": ["python", "-m", "tests.runner.run_y2s4_suite", *sys.argv[1:]],
    ...
    print(f"{'passed' if passed else 'failed'}: {len(rows)} Y2S4 scenarios; artifact={output.relative_to(ROOT)}")

...
TEST_MODULE = "tests.semantic.test_a6_hardening_contract"
REFERENCE_PROFILE_ID = "a6_mvp_a_reference_v1"
...
def blocked_socket(*args: Any, **kwargs: Any) -> Any:
    raise RuntimeError("external network is disabled by the A6 runner")
...
def main() -> int:
    ...
    previous_adapter = os.environ.get("NOETIDE_A6_ADAPTER")
    ...
    try:
        os.environ["NOETIDE_A6_ADAPTER"] = args.adapter
        ...
        if previous_adapter is None:
            os.environ.pop("NOETIDE_A6_ADAPTER", None)
        else:
            os.environ["NOETIDE_A6_ADAPTER"] = previous_adapter
    monotonic_duration = time.perf_counter() - mono_start
    ...
    artifact = {
        "schema_version": "noetide.a6-run-result.v1",
        "run_id": f"a6-{uuid.uuid4().hex}",
        ...
        "reference_profile": {
            ...
            "slo_scope_note": "SLO observations are valid only for this profile; cross-profile extrapolation is forbidden"
        },
        "environment": {
            ...
            "wall_time_utc": wall_start.isoformat(),
            "monotonic_duration_seconds": round(monotonic_duration, 6),
            "timezone": time.tzname[0]
        },
        "command": ["python", "-m", "tests.runner.run_a6_suite", *sys.argv[1:]],
    ...
    output.write_text(json.dumps(artifact, ensure_ascii=False, indent=2) + chr(10), encoding="utf-8", newline=chr(10))
    print(f"{'passed' if passed else 'failed'}: {len(rows)} A6 scenarios; artifact={output.relative_to(ROOT)}")

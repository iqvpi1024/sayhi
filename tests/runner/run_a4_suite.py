...
TEST_MODULE = "tests.semantic.test_a4_access_policy_contract"
...
def blocked_socket(*args: Any, **kwargs: Any) -> Any:
    raise RuntimeError("external network is disabled by the A4 runner")
...
def main() -> int:
    ...
    previous_adapter = os.environ.get("NOETIDE_A4_ADAPTER")
    ...
    try:
        os.environ["NOETIDE_A4_ADAPTER"] = args.adapter
        ...
        if previous_adapter is None:
            os.environ.pop("NOETIDE_A4_ADAPTER", None)
        else:
            os.environ["NOETIDE_A4_ADAPTER"] = previous_adapter
    ...
    artifact = {
        "schema_version": "noetide.a4-run-result.v1",
        "run_id": f"a4-{uuid.uuid4().hex}",
        ...
        "command": ["python", "-m", "tests.runner.run_a4_suite", *sys.argv[1:]],
    ...
    print(f"{'passed' if passed else 'failed'}: {len(rows)} A4 scenarios; artifact={output.relative_to(ROOT)}")

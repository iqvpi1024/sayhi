...
TEST_MODULE = "tests.semantic.test_b6_shadow_migration_contract"
...
def blocked_socket(*args: Any, **kwargs: Any) -> Any:
    raise RuntimeError("external network is disabled by the B6 runner")
...
def main() -> int:
    ...
    previous_adapter = os.environ.get("NOETIDE_B6_ADAPTER")
    ...
    try:
        os.environ["NOETIDE_B6_ADAPTER"] = args.adapter
        ...
        if previous_adapter is None:
            os.environ.pop("NOETIDE_B6_ADAPTER", None)
        else:
            os.environ["NOETIDE_B6_ADAPTER"] = previous_adapter
    ...
    artifact = {
        "schema_version": "noetide.b6-run-result.v1",
        "run_id": f"b4-{uuid.uuid4().hex}",
        ...
        "command": ["python", "-m", "tests.runner.run_b6_suite", *sys.argv[1:]],
    ...
    print(f"{'passed' if passed else 'failed'}: {len(rows)} B6 scenarios; artifact={output.relative_to(ROOT)}")

...
TEST_MODULE = "tests.semantic.test_a5_app_shell_contract"
...
def blocked_socket(*args: Any, **kwargs: Any) -> Any:
    raise RuntimeError("external network is disabled by the A5 runner")
...
def main() -> int:
    ...
    previous_adapter = os.environ.get("NOETIDE_A5_ADAPTER")
    ...
    try:
        os.environ["NOETIDE_A5_ADAPTER"] = args.adapter
        ...
        if previous_adapter is None:
            os.environ.pop("NOETIDE_A5_ADAPTER", None)
        else:
            os.environ["NOETIDE_A5_ADAPTER"] = previous_adapter
    ...
    artifact = {
        "schema_version": "noetide.a5-run-result.v1",
        "run_id": f"a5-{uuid.uuid4().hex}",
        ...
        "command": ["python", "-m", "tests.runner.run_a5_suite", *sys.argv[1:]],
    ...
    print(f"{'passed' if passed else 'failed'}: {len(rows)} A5 scenarios; artifact={output.relative_to(ROOT)}")

...
TEST_MODULE = "tests.semantic.test_y2s5_mcp_runtime_contract"
...
class LoopbackOnlySocket(_ORIGINAL_SOCKET):
    def connect(self, address: Any) -> None:
        if not _is_loopback_address(address):
            raise RuntimeError("external network is disabled by the Y2S5 runner")
...
def main() -> int:
    ...
    previous_adapter = os.environ.get("NOETIDE_Y2S5_ADAPTER")
    socket.socket, socket.create_connection = LoopbackOnlySocket, loopback_create_connection
    try:
        os.environ["NOETIDE_Y2S5_ADAPTER"] = args.adapter
        ...
        if previous_adapter is None:
            os.environ.pop("NOETIDE_Y2S5_ADAPTER", None)
        else:
            os.environ["NOETIDE_Y2S5_ADAPTER"] = previous_adapter
    ...
    artifact = {
        "schema_version": "noetide.y2s5-run-result.v1",
        "run_id": f"y2s5-{uuid.uuid4().hex}",
        ...
        "command": ["python", "-m", "tests.runner.run_y2s5_suite", *sys.argv[1:]],
    ...
    print(f"{'passed' if passed else 'failed'}: {len(rows)} Y2S5 scenarios; artifact={output.relative_to(ROOT)}")

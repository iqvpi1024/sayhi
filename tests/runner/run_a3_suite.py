...
TEST_MODULE = "tests.semantic.test_a3_entity_merge_contract"
...
def blocked_socket(*args: Any, **kwargs: Any) -> Any:
    raise RuntimeError("external network is disabled by the A3 runner")
...
def main() -> int:
    ...
    previous_adapter = os.environ.get("NOETIDE_A3_ADAPTER")
    ...
    try:
        os.environ["NOETIDE_A3_ADAPTER"] = args.adapter
        ...
        if previous_adapter is None:
            os.environ.pop("NOETIDE_A3_ADAPTER", None)
        else:
            os.environ["NOETIDE_A3_ADAPTER"] = previous_adapter
    ...
    artifact = {
        "schema_version": "noetide.a3-run-result.v1",
        "run_id": f"a3-{uuid.uuid4().hex}",
        ...
        "command": ["python", "-m", "tests.runner.run_a3_suite", *sys.argv[1:]],
    ...
    print(f"{'passed' if passed else 'failed'}: {len(rows)} A3 scenarios; artifact={output.relative_to(ROOT)}")

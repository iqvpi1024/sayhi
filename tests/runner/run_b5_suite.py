...
TEST_MODULE = "tests.semantic.test_b5_multilingual_contract"
...
def blocked_socket(*args: Any, **kwargs: Any) -> Any:
    raise RuntimeError("external network is disabled by the B5 runner")
...
def main() -> int:
    ...
    previous_adapter = os.environ.get("NOETIDE_B5_ADAPTER")
    ...
    try:
        os.environ["NOETIDE_B5_ADAPTER"] = args.adapter
        ...
        if previous_adapter is None:
            os.environ.pop("NOETIDE_B5_ADAPTER", None)
        else:
            os.environ["NOETIDE_B5_ADAPTER"] = previous_adapter
    ...
    artifact = {
        "schema_version": "noetide.b5-run-result.v1",
        ...
        "command": ["python", "-m", "tests.runner.run_b5_suite", *sys.argv[1:]],
    ...
    print(f"{'passed' if passed else 'failed'}: {len(rows)} B5 scenarios; artifact={output.relative_to(ROOT)}")

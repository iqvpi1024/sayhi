...
TEST_MODULE = "tests.semantic.test_a2_current_state_contract"
...
def blocked_socket(*args: Any, **kwargs: Any) -> Any:
    raise RuntimeError("external network is disabled by the A2 runner")
...
def main() -> int:
    ...
    previous_adapter = os.environ.get("NOETIDE_A2_ADAPTER")
    ...
    try:
        os.environ["NOETIDE_A2_ADAPTER"] = args.adapter
        ...
        if previous_adapter is None:
            os.environ.pop("NOETIDE_A2_ADAPTER", None)
        else:
            os.environ["NOETIDE_A2_ADAPTER"] = previous_adapter
    ...
    artifact = {
        "schema_version": "noetide.a2-run-result.v1",
        "run_id": f"a2-{uuid.uuid4().hex}",
        ...
        "command": ["python", "-m", "tests.runner.run_a2_suite", *sys.argv[1:]],
    ...
    print(f"{'passed' if passed else 'failed'}: {len(rows)} A2 scenarios; artifact={output.relative_to(ROOT)}")

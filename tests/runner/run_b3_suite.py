...
TEST_MODULE = "tests.semantic.test_b3_commitment_contract"
...
def blocked_socket(*args: Any, **kwargs: Any) -> Any:
    raise RuntimeError("external network is disabled by the B3 runner")
...
def main() -> int:
    ...
    previous_adapter = os.environ.get("NOETIDE_B3_ADAPTER")
    ...
    try:
        os.environ["NOETIDE_B3_ADAPTER"] = args.adapter
        ...
        if previous_adapter is None:
            os.environ.pop("NOETIDE_B3_ADAPTER", None)
        else:
            os.environ["NOETIDE_B3_ADAPTER"] = previous_adapter
    ...
    artifact = {
        "schema_version": "noetide.b3-run-result.v1",
        "run_id": f"b3-{uuid.uuid4().hex}",
        ...
        "command": ["python", "-m", "tests.runner.run_b3_suite", *sys.argv[1:]],
    ...
    print(f"{'passed' if passed else 'failed'}: {len(rows)} B3 scenarios; artifact={output.relative_to(ROOT)}")

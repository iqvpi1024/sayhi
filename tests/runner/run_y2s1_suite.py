...
TEST_MODULE = "tests.semantic.test_y2s1_folder_import_contract"
...
def blocked_socket(*args: Any, **kwargs: Any) -> Any:
    raise RuntimeError("external network is disabled by the Y2S1 runner")
...
def main() -> int:
    ...
    previous_adapter = os.environ.get("NOETIDE_Y2S1_ADAPTER")
    ...
    try:
        os.environ["NOETIDE_Y2S1_ADAPTER"] = args.adapter
        ...
        if previous_adapter is None:
            os.environ.pop("NOETIDE_Y2S1_ADAPTER", None)
        else:
            os.environ["NOETIDE_Y2S1_ADAPTER"] = previous_adapter
    ...
    artifact = {
        "schema_version": "noetide.y2s1-run-result.v1",
        "run_id": f"y2s1-{uuid.uuid4().hex}",
        ...
        "command": ["python", "-m", "tests.runner.run_y2s1_suite", *sys.argv[1:]],
    ...
    print(f"{'passed' if passed else 'failed'}: {len(rows)} Y2S1 scenarios; artifact={output.relative_to(ROOT)}")

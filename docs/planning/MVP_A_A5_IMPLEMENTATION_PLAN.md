...
|---|---|---|---|---|
| `A5-TASK-001` | `app_shell.py` 呈现层纯函数（NL review、impact preview、summary line）与零绕过静态扫描辅助 | §2-§5、`A5-002/003/008` | 呈现输出形状确定；壳模块无 store 写调用可静态证明 | `pending` |
| `A5-TASK-002` | 壳命令接线（cli.py 增加 guide/receipts/history 命令）与 `a5_testing_adapter.py` | §3、§7-§8、`A5-001..008` | adapter 完整实现 protocol；fixture/oracle 不被修改 | `pending` |

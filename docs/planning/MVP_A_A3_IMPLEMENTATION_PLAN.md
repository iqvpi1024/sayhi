...
|---|---|---|---|---|
| `A3-TASK-001` | store 的 merge_records 持久化辅助（表、写入/读取、只增不改） | §2、§6、`A3-001/006` | merge_record 原子写入、完整读取、PRAGMA 行为可测 | `pending` |
| `A3-TASK-002` | `entity_merge.py`：merge/split ChangeSet 服务（preflight、原子重定向、split 等价恢复、fail closed） | §2-§7、`A3-001..008` | 原子性可注入失败证明；split 逐字段恢复等价；trust/closeness/人格不变 | `pending` |

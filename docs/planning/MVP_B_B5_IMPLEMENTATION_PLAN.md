...
| `B5-TASK-002` | `b5_testing_adapter.py` 与 B5 contract 集成 | §7/§8、`B5-001..008` | adapter 完整实现 protocol；fixture/oracle 不被修改；B5-008 横切通过 | `completed`；contract 8/8 passed（adapter），见 `b5-task002-20260725.json` |
| `B5-TASK-003` | B5 official runner、existing regression 与 immutable result | §7/§8 | B5 8/8 同一次 run passed；既有 suite 无回归；manifest 正确绑定 result | `completed`；`b5-20260725.json` 8/8 current/passed，全量回归 309 OK 0 skip |
| `B5-TASK-004` | Gate Review、状态/追踪、Recovery Point | Process 流程 | P0/P1=0、验证记录完整、tag 仅在审查通过后创建 | `completed`；Gate Review `B5_MULTILINGUAL_GATE_REVIEW_2026-07-25.md` P0=0/P1=0，recovery tag `b5-multilingual-rp-20260725` |

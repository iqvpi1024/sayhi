...
- `PRD-§24.5 Y2-S1`、`FR-302`：批量导入 3 有效文件 stored、字段齐全、报告计数正确 -> `SPEC-Y2S1-FOLDER-IMPORT-001` §2-§7；S1 §6.2；S9 §4-§7 -> `Y2S1-001` -> `noetide_micro.folder_import` / `noetide_micro.y2s1_testing_adapter` -> `passed`（y2s1-20260801.json）
- `PRD-§9.2`、`§19.4`：每文件哈希、字节长度、来源时间、CoverageWindow、语言逐项核对 -> `SPEC-Y2S1-FOLDER-IMPORT-001` §2.1-§2.2；S9 §6 -> `Y2S1-002` -> `noetide_micro.folder_import` / `noetide_micro.y2s1_testing_adapter` -> `passed`（y2s1-20260801.json）
- `PRD-§21.1`：重复内容不同文件名，duplicate 指向既有 source_id -> `SPEC-Y2S1-FOLDER-IMPORT-001` §2.2/§8 -> `Y2S1-003` -> `noetide_micro.folder_import` / `noetide_micro.y2s1_testing_adapter` -> `passed`（y2s1-20260801.json）
- `PRD-§21.1`：重复扫描零新 Source -> `SPEC-Y2S1-FOLDER-IMPORT-001` §3/§8 -> `Y2S1-004` -> `noetide_micro.folder_import` / `noetide_micro.y2s1_testing_adapter` -> `passed`（y2s1-20260801.json）
- `PRD-§24.5`：非白名单扩展名 skipped、计数正确、其余正常 -> `SPEC-Y2S1-FOLDER-IMPORT-001` §3/§5 -> `Y2S1-005` -> `noetide_micro.folder_import` / `noetide_micro.y2s1_testing_adapter` -> `passed`（y2s1-20260801.json）
- `PRD-§21.1`、`§25.1`：根外/穿越/逃逸 rejected 零写入 -> `SPEC-Y2S1-FOLDER-IMPORT-001` §5/§8 -> `Y2S1-006` -> `noetide_micro.folder_import` / `noetide_micro.y2s1_testing_adapter` -> `passed`（y2s1-20260801.json）
- `PRD-§25.2`：无效 UTF-8 rejected(invalid_utf8)，其余正常 -> `SPEC-Y2S1-FOLDER-IMPORT-001` §5/§8 -> `Y2S1-007` -> `noetide_micro.folder_import` / `noetide_micro.y2s1_testing_adapter` -> `passed`（y2s1-20260801.json）
- `PRD-§21.6.5`：注入中断后重跑，终态与无中断一致 -> `SPEC-Y2S1-FOLDER-IMPORT-001` §6/§8 -> `Y2S1-008` -> `noetide_micro.folder_import` / `noetide_micro.y2s1_testing_adapter` -> `passed`（y2s1-20260801.json）
- `PRD-§24.5`：两次 poll，第一次导入新增、第二次零新增、游标只前进 -> `SPEC-Y2S1-FOLDER-IMPORT-001` §2.4/§3 -> `Y2S1-009` -> `noetide_micro.folder_import` / `noetide_micro.y2s1_testing_adapter` -> `passed`（y2s1-20260801.json）
- `PRD-§11.1`、`§21.6`：横切——Canonical digest/data_revision/Core View 不变、报告确定性、profile 外 fail closed -> `SPEC-Y2S1-FOLDER-IMPORT-001` §4/§7/§8 -> `Y2S1-010` -> `noetide_micro.folder_import` / `noetide_micro.y2s1_testing_adapter` -> `passed`（y2s1-20260801.json）

状态：`product_decided=true`（`DEC-Y2-S1-001`，2026-08-01）、`spec_applicability=pass_with_slice_contract_required`（`Y2S1-SPEC-APPLICABILITY-001`，2026-08-01）、`spec_approved=true`（`Y2S1-CONTRACT-REVIEW-001`，2026-08-01）、`traceable=true`、`adr_accepted=true`（`ADR-0020`，2026-08-01）、`suite_defined=true`、`suite_materialized=true`（2026-08-01）、`suite_executed=true`、`suite_passed=true`（2026-08-01，official runner 同一次 run 10/10 passed，immutable result `docs/testing/results/y2s1-20260801.json`，manifest 已绑定）、`gate_review_passed=true`（`Y2_S1_FOLDER_IMPORT_GATE_REVIEW_2026-08-01.md`，P0=0/P1=0）、`verified=true`（recovery tag `y2s1-folder-import-rp-20260801`）。

## 4.22 Active Slice：Y2-S2 本地模型提议式整理
...
## 4.24 Active Slice：Y2-S4 云端模型可选后端

`SLICE-Y2-S4-CLOUD-MODEL-001` 只实现 PRDv06 §14.5.3 的云端可选后端首片（`DEC-Y2-ENTRY-001`/`DEC-Y2-S4-001`）：默认禁用、按舱室+目的显式授权、红线 fail closed、外发预览、Ledger 审计、propose-only 候选。它不代表 MCP（Y2-S5）、账户、真实数据模式或自动上传。

- `PRD-§14.5.3`、`§17.4`、`§24.5 Y2-S4`：无有效授权时整批 default_disabled 拒绝、零后端调用 -> `SPEC-Y2S4-CLOUD-MODEL-001` §3/§5/§7；S4/S5 -> `Y2S4-001` -> `noetide_micro.cloud_model` / `noetide_micro.y2s4_testing_adapter` -> `passed`（y2s4-20260803.json）
- `PRD-§14.5.3`、`§17.4`：有效 grant + preview 后云端 fixture 可 propose；候选全 unconfirmed、Canonical/revision 不变 -> `SPEC-Y2S4-CLOUD-MODEL-001` §3/§5/§7；S4/S5 -> `Y2S4-002` -> `noetide_micro.cloud_model` / `noetide_micro.y2s4_testing_adapter` -> `passed`（y2s4-20260803.json）
- `PRD-§14.5.3`、`§26 Case H`：红线舱室 health/finance/relationship/sealed 即使有授权也整批 red_line_denied、零调用 -> `SPEC-Y2S4-CLOUD-MODEL-001` §3/§5/§7；S4/S5 -> `Y2S4-003` -> `noetide_micro.cloud_model` / `noetide_micro.y2s4_testing_adapter` -> `passed`（y2s4-20260803.json）
- `PRD-§14.5.3`、`§17.4`：purpose 不匹配整批拒绝 -> `SPEC-Y2S4-CLOUD-MODEL-001` §3/§5/§7；S4 -> `Y2S4-004` -> `noetide_micro.cloud_model` / `noetide_micro.y2s4_testing_adapter` -> `passed`（y2s4-20260803.json）
- `PRD-§14.5.3`、`§17.4`：source scope 不匹配整批拒绝 -> `SPEC-Y2S4-CLOUD-MODEL-001` §3/§5/§7；S4 -> `Y2S4-005` -> `noetide_micro.cloud_model` / `noetide_micro.y2s4_testing_adapter` -> `passed`（y2s4-20260803.json）
- `PRD-§17.4`：到期与撤销授权立即拒绝 -> `SPEC-Y2S4-CLOUD-MODEL-001` §3/§5/§7；S4 -> `Y2S4-006` -> `noetide_micro.cloud_model` / `noetide_micro.y2s4_testing_adapter` -> `passed`（y2s4-20260803.json）
- `PRD-§17.4`、`§25.1`：无预览/预览不匹配即拒绝，预览不含原始正文 -> `SPEC-Y2S4-CLOUD-MODEL-001` §2/§3/§5/§7；S4 -> `Y2S4-007` -> `noetide_micro.cloud_model` / `noetide_micro.y2s4_testing_adapter` -> `passed`（y2s4-20260803.json）
- `PRD-§14.5.3`、`§25.2`：畸形输出与传输失败零候选、失败审计存在 -> `SPEC-Y2S4-CLOUD-MODEL-001` §3/§5/§6/§7；S5/S6 -> `Y2S4-008` -> `noetide_micro.cloud_model` / `noetide_micro.y2s4_testing_adapter` -> `passed`（y2s4-20260803.json）
- `PRD-§22.4`、`§14.5.3`：审计事件、候选 provenance 与版本注册/回滚 -> `SPEC-Y2S4-CLOUD-MODEL-001` §3/§4/§5/§7；S5/S7 -> `Y2S4-009` -> `noetide_micro.cloud_model` / `noetide_micro.y2s4_testing_adapter` -> `passed`（y2s4-20260803.json）
- `PRD-§14.5.3`、`§17.4`、`§24.5`：横切——确定性、loopback-only、stdlib only、fixture 显式合成、profile fail closed -> `SPEC-Y2S4-CLOUD-MODEL-001` §5/§7；S6/S7 -> `Y2S4-010` -> `noetide_micro.cloud_model` / `noetide_micro.y2s4_testing_adapter` -> `passed`（y2s4-20260803.json）

状态：`product_decided=true`（`DEC-Y2-S4-001`，2026-08-03）、`spec_applicability=pass_with_slice_contract_required`（`Y2S4-SPEC-APPLICABILITY-001`，2026-08-03）、`slice_contract=SPEC-Y2S4-CLOUD-MODEL-001 v0.1`、`traceable=true`、`adr_accepted=true`（`ADR-0023`，2026-08-03）、`suite_defined=true`、`suite_materialized=true`（2026-08-03）、`suite_executed=true`、`suite_passed=true`（2026-08-03，official runner 同一次 run 10/10 passed，immutable result `docs/testing/results/y2s4-20260803.json`，manifest 已绑定）、`gate_review_passed=true`（`Y2_S4_CLOUD_MODEL_GATE_REVIEW_2026-08-03.md`，P0=0/P1=0）、`verified=true`（recovery tag `y2s4-cloud-model-rp-20260803`）。

## 4.25 Active Slice：Y2-S5 MCP runtime 最小子集

`SLICE-Y2-S5-MCP-RUNTIME-001` 只实现 PRDv06 §19.5 的 MCP runtime 最小子集首片（`DEC-Y2-ENTRY-001`/`DEC-Y2-S5-001`，`DQ-013` decided）：本地 127.0.0.1、显式 capability、默认关闭、只读/propose/append、幂等、审计、红线/sealed fail closed、不可逆动作无例外拒绝。它不代表完整 MCP、A2A、多 Agent、账户、真实数据模式、大文件传输或云端调用。

- `PRD-§19.5`、`§17.4`、`§24.5`：default closed、S8 envelope、唯一 denied profile、显式 capability -> `SPEC-Y2S5-MCP-RUNTIME-001` §3/§5/§7；S8 -> `Y2S5-001`/`Y2S5-002`/`Y2S5-010` -> `noetide_micro.mcp_runtime` / `noetide_micro.y2s5_testing_adapter` -> `passed`（y2s5-20260803.json）
- `PRD-§19.1`、`§19.2`、`§17.4`：最小读、字段 redaction、sealed 直查 fail closed -> `SPEC-Y2S5-MCP-RUNTIME-001` §3/§5/§7；S8 -> `Y2S5-003` -> `noetide_micro.mcp_runtime` / `noetide_micro.y2s5_testing_adapter` -> `passed`（y2s5-20260803.json）
- `PRD-§19.2`、`§22.4`：propose changeset 只写 Ledger proposed receipt，Canonical/revision 不变 -> `SPEC-Y2S5-MCP-RUNTIME-001` §3/§4/§5/§7；S8 -> `Y2S5-004`/`Y2S5-007` -> `noetide_micro.mcp_runtime` / `noetide_micro.y2s5_testing_adapter` -> `passed`（y2s5-20260803.json）
- `PRD-§19.2`、`§22.4`：record source 只追加 Source receipt，Canonical/revision 不变 -> `SPEC-Y2S5-MCP-RUNTIME-001` §3/§5/§7；S8 -> `Y2S5-005`/`Y2S5-006` -> `noetide_micro.mcp_runtime` / `noetide_micro.y2s5_testing_adapter` -> `passed`（y2s5-20260803.json）
- `PRD-§17.4`、`§19.5`：同 key 幂等/冲突与 revision precondition -> `SPEC-Y2S5-MCP-RUNTIME-001` §3/§7；S8 -> `Y2S5-006`/`Y2S5-007` -> `noetide_micro.mcp_runtime` / `noetide_micro.y2s5_testing_adapter` -> `passed`（y2s5-20260803.json）
- `PRD-§19.5`、`§26 Case H`、`DQ-013`：不可逆工具不存在且任何请求 denied、零写、`DQ-013` 无例外 -> `SPEC-Y2S5-MCP-RUNTIME-001` §3/§5/§6/§7；S8 -> `Y2S5-008` -> `noetide_micro.mcp_runtime` / `noetide_micro.y2s5_testing_adapter` -> `passed`（y2s5-20260803.json）
- `PRD-§19.1`、`§19.2`：畸形请求、未知工具、policy unavailable、超限大文件全部 fail closed 或返回 import reference，零 Canonical 写 -> `SPEC-Y2S5-MCP-RUNTIME-001` §3/§6/§7；S8 -> `Y2S5-009` -> `noetide_micro.mcp_runtime` / `noetide_micro.y2s5_testing_adapter` -> `passed`（y2s5-20260803.json）
- `PRD-§19.5`、`§17.4`、`§24.5`：横切——确定性、stdlib only、127.0.0.1 only、fixture 显式合成、profile fail closed -> `SPEC-Y2S5-MCP-RUNTIME-001` §4/§5/§7；S8 -> `Y2S5-010` -> `noetide_micro.mcp_runtime` / `noetide_micro.y2s5_testing_adapter` -> `passed`（y2s5-20260803.json）

状态：`product_decided=true`（`DEC-Y2-S5-001`，2026-08-03）、`spec_applicability=pass_with_slice_contract_required`（`Y2S5-SPEC-APPLICABILITY-001`，2026-08-03）、`slice_contract=SPEC-Y2S5-MCP-RUNTIME-001 v0.1`、`traceable=true`、`adr_accepted=true`（`ADR-0024`，2026-08-03）、`suite_defined=true`、`suite_materialized=true`（2026-08-03）、`suite_executed=true`、`suite_passed=true`（2026-08-03，official runner 同一次 run 10/10 passed，immutable result `docs/testing/results/y2s5-20260803.json`，manifest 已绑定）、`gate_review_passed=true`（`Y2_S5_MCP_RUNTIME_GATE_REVIEW_2026-08-03.md`，P0=0/P1=0）、`verified=true`（recovery tag `y2s5-mcp-runtime-rp-20260803`）。

...
MANIFEST = ROOT / "tests/b3_suite_manifest.json"
REQUIRED = [f"B3-{index:03d}" for index in range(1, 9)]
...
def main() -> int:
    ...
    if flags not in (materialized_flags, passed_flags):
        errors.append("invalid B3 suite flags")
    ...
    fixture = json.loads((ROOT / "tests/fixtures/b3_commitment_v1/fixture.json").read_text(encoding="utf-8"))
    oracle = json.loads((ROOT / "tests/fixtures/b3_commitment_v1/oracles.json").read_text(encoding="utf-8"))
    scenarios = json.loads((ROOT / "tests/integration/b3_commitment_scenarios.json").read_text(encoding="utf-8"))
    ...
    if fixture.get("synthetic_profile_id") != "b3_commitment_v1":
    ...
    test_path = ROOT / "tests/semantic/test_b3_commitment_contract.py"
    ...
    if errors:
        ...
        return 1
    print("PASSED: B3 Commitment suite materialized and current business runner result is bound" if flags == passed_flags else "PASSED: B3 Commitment suite materialized; no business test was executed")
    return 0

...
MANIFEST = ROOT / "tests/y2s4_suite_manifest.json"
REQUIRED = [f"Y2S4-{index:03d}" for index in range(1, 11)]
...
def main() -> int:
    ...
    if flags not in (materialized_flags, passed_flags):
        errors.append("invalid Y2S4 suite flags")
    ...
    fixture = json.loads((ROOT / "tests/fixtures/y2s4_cloud_model_v1/fixture.json").read_text(encoding="utf-8"))
    oracle = json.loads((ROOT / "tests/fixtures/y2s4_cloud_model_v1/oracles.json").read_text(encoding="utf-8"))
    scenarios = json.loads((ROOT / "tests/integration/y2s4_cloud_model_scenarios.json").read_text(encoding="utf-8"))
    ...
    if fixture.get("synthetic_profile_id") != "y2s4_cloud_model_v1":
    ...
    test_path = ROOT / "tests/semantic/test_y2s4_cloud_model_contract.py"
    ...
    if errors:
        for error in errors:
            print(f"ERROR: {error}")
        return 1
    print("PASSED: Y2S4 suite materialized and current business runner result is bound" if flags == passed_flags else "PASSED: Y2S4 suite materialization preflight; no business test was executed")
    return 0

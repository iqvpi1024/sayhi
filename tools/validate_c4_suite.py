...
REQUIRED = [f"C4-{index:03d}" for index in range(1, 11)]
...
def main() -> int:
    ...
    if flags not in (materialized_flags, passed_flags):
        errors.append("invalid C4 suite flags")
    ...
    oracle = json.loads((ROOT / "tests/fixtures/c4_scenario_action_v1/oracles.json").read_text(encoding="utf-8"))
    scenarios = json.loads((ROOT / "tests/integration/c4_scenario_scenarios.json").read_text(encoding="utf-8"))
    ...
    if fixture.get("synthetic_profile_id") != "c4_scenario_action_v1":
    ...
    test_path = ROOT / "tests/semantic/test_c4_scenario_contract.py"
    ...
    if errors:
        ...
        return 1
    print("PASSED: C4 Scenario & Action suite materialized and current business runner result is bound" if flags == passed_flags else "PASSED: C4 Scenario & Action suite materialized; no business test was executed")
    return 0

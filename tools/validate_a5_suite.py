...
MANIFEST = ROOT / "tests/a5_suite_manifest.json"
REQUIRED = [f"A5-{index:03d}" for index in range(1, 9)]
...
def main() -> int:
    ...
    if flags not in (materialized_flags, passed_flags):
        errors.append("invalid A5 suite flags")
    ...
    oracle = json.loads((ROOT / "tests/fixtures/a5_app_shell_v1/oracles.json").read_text(encoding="utf-8"))
    scenarios = json.loads((ROOT / "tests/integration/a5_app_shell_scenarios.json").read_text(encoding="utf-8"))
    ...
    if fixture.get("synthetic_profile_id") != "a5_app_shell_v1":
    ...
    test_path = ROOT / "tests/semantic/test_a5_app_shell_contract.py"
    ...
    if errors:
        ...
        return 1
    print("PASSED: A5 App Shell suite materialized and current business runner result is bound" if flags == passed_flags else "PASSED: A5 App Shell suite materialized; no business test was executed")
    return 0

...
MANIFEST = ROOT / "tests/a2_suite_manifest.json"
REQUIRED = [f"A2-{index:03d}" for index in range(1, 9)]
...
def main() -> int:
    ...
    if flags not in (materialized_flags, passed_flags):
        errors.append("invalid A2 suite flags")
    ...
    fixture = json.loads((ROOT / "tests/fixtures/a2_current_state_v1/fixture.json").read_text(encoding="utf-8"))
    oracle = json.loads((ROOT / "tests/fixtures/a2_current_state_v1/oracles.json").read_text(encoding="utf-8"))
    scenarios = json.loads((ROOT / "tests/integration/a2_current_state_scenarios.json").read_text(encoding="utf-8"))
    ...
    if fixture.get("synthetic_profile_id") != "a2_current_state_v1":
    ...
    test_path = ROOT / "tests/semantic/test_a2_current_state_contract.py"
    ...
    if errors:
        ...
        return 1
    print("PASSED: A2 CurrentState suite materialized and current business runner result is bound" if flags == passed_flags else "PASSED: A2 CurrentState suite materialized; no business test was executed")
    return 0

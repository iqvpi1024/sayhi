...
MANIFEST = ROOT / "tests/b4_suite_manifest.json"
REQUIRED = [f"B4-{index:03d}" for index in range(1, 11)]
...
def main() -> int:
    ...
    if flags not in (materialized_flags, passed_flags):
        errors.append("invalid B4 suite flags")
    ...
    fixture = json.loads((ROOT / "tests/fixtures/b4_reconciliation_v1/fixture.json").read_text(encoding="utf-8"))
    oracle = json.loads((ROOT / "tests/fixtures/b4_reconciliation_v1/oracles.json").read_text(encoding="utf-8"))
    scenarios = json.loads((ROOT / "tests/integration/b4_reconciliation_scenarios.json").read_text(encoding="utf-8"))
    ...
    if fixture.get("synthetic_profile_id") != "b4_reconciliation_v1":
    ...
    test_path = ROOT / "tests/semantic/test_b4_reconciliation_contract.py"
    ...
    if errors:
        ...
        return 1
    print("PASSED: B4 Reconciliation suite materialized and current business runner result is bound" if flags == passed_flags else "PASSED: B4 Reconciliation suite materialized; no business test was executed")
    return 0

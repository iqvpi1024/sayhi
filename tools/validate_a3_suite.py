...
MANIFEST = ROOT / "tests/a3_suite_manifest.json"
REQUIRED = [f"A3-{index:03d}" for index in range(1, 9)]
...
def main() -> int:
    ...
    if flags not in (materialized_flags, passed_flags):
        errors.append("invalid A3 suite flags")
    ...
    fixture = json.loads((ROOT / "tests/fixtures/a3_entity_merge_v1/fixture.json").read_text(encoding="utf-8"))
    oracle = json.loads((ROOT / "tests/fixtures/a3_entity_merge_v1/oracles.json").read_text(encoding="utf-8"))
    scenarios = json.loads((ROOT / "tests/integration/a3_entity_merge_scenarios.json").read_text(encoding="utf-8"))
    ...
    if fixture.get("synthetic_profile_id") != "a3_entity_merge_v1":
    ...
    test_path = ROOT / "tests/semantic/test_a3_entity_merge_contract.py"
    ...
    if errors:
        ...
        return 1
    print("PASSED: A3 Entity Merge suite materialized and current business runner result is bound" if flags == passed_flags else "PASSED: A3 Entity Merge suite materialized; no business test was executed")
    return 0

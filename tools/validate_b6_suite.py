...
MANIFEST = ROOT / "tests/b6_suite_manifest.json"
REQUIRED = [f"B6-{index:03d}" for index in range(1, 11)]
...
def main() -> int:
    ...
    if flags not in (materialized_flags, passed_flags):
        errors.append("invalid B6 suite flags")
    ...
    fixture = json.loads((ROOT / "tests/fixtures/b6_shadow_migration_v1/fixture.json").read_text(encoding="utf-8"))
    oracle = json.loads((ROOT / "tests/fixtures/b6_shadow_migration_v1/oracles.json").read_text(encoding="utf-8"))
    scenarios = json.loads((ROOT / "tests/integration/b6_shadow_migration_scenarios.json").read_text(encoding="utf-8"))
    ...
    if fixture.get("synthetic_profile_id") != "b6_shadow_migration_v1":
    ...
    test_path = ROOT / "tests/semantic/test_b6_shadow_migration_contract.py"
    ...
    if errors:
        ...
        return 1
    print("PASSED: B6 Shadow Migration suite materialized and current business runner result is bound" if flags == passed_flags else "PASSED: B6 Shadow Migration suite materialized; no business test was executed")
    return 0

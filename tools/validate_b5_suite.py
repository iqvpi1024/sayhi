...
MANIFEST = ROOT / "tests/b5_suite_manifest.json"
REQUIRED = [f"B5-{index:03d}" for index in range(1, 9)]
...
def main() -> int:
    ...
    if flags not in (materialized_flags, passed_flags):
        errors.append("invalid B5 suite flags")
    ...
    fixture = json.loads((ROOT / "tests/fixtures/b5_multilingual_v1/fixture.json").read_text(encoding="utf-8"))
    oracle = json.loads((ROOT / "tests/fixtures/b5_multilingual_v1/oracles.json").read_text(encoding="utf-8"))
    scenarios = json.loads((ROOT / "tests/integration/b5_multilingual_scenarios.json").read_text(encoding="utf-8"))
    ...
    if fixture.get("synthetic_profile_id") != "b5_multilingual_v1":
    ...
    test_path = ROOT / "tests/semantic/test_b5_multilingual_contract.py"
    ...
    if errors:
        ...
        return 1
    print("PASSED: B5 Multilingual suite materialized and current business runner result is bound" if flags == passed_flags else "PASSED: B5 Multilingual suite materialized; no business test was executed")
    return 0

...
MANIFEST = ROOT / "tests/a6_suite_manifest.json"
REQUIRED = [f"A6-{index:03d}" for index in range(1, 22)]
...
def main() -> int:
    ...
    if flags not in (materialized_flags, passed_flags):
        errors.append("invalid A6 suite flags")
    ...
    fixture = json.loads((ROOT / "tests/fixtures/a6_hardening_v1/fixture.json").read_text(encoding="utf-8"))
    oracle = json.loads((ROOT / "tests/fixtures/a6_hardening_v1/oracles.json").read_text(encoding="utf-8"))
    scenarios = json.loads((ROOT / "tests/integration/a6_hardening_scenarios.json").read_text(encoding="utf-8"))
    ...
    if fixture.get("synthetic_profile_id") != "a6_hardening_v1":
        errors.append("fixture synthetic profile mismatch")
    if fixture.get("reference_profile", {}).get("profile_id") != "a6_mvp_a_reference_v1":
        errors.append("reference profile ID mismatch")
    ...
    if scenarios.get("execution", {}).get("order") != REQUIRED:
        errors.append("scenario execution order mismatch")
    test_path = ROOT / "tests/semantic/test_a6_hardening_contract.py"
    ...
    if flags == passed_flags:
        ...
        if not result_path.is_file() or digest(result_path) != manifest.get("latest_verification_result_sha256"):
            ...
            if result.get("reference_profile", {}).get("profile_id") != "a6_mvp_a_reference_v1":
                errors.append("current result reference profile mismatch")
    ...
    if errors:
        ...
        return 1
    print("PASSED: A6 Hardening suite materialized and current business runner result is bound" if flags == passed_flags else "PASSED: A6 Hardening suite materialized; no business test was executed")
    return 0

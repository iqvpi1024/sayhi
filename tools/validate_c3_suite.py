...
MANIFEST = ROOT / "tests/c3_suite_manifest.json"
REQUIRED = [f"C3-{index:03d}" for index in range(1, 11)]
...
def main() -> int:
    ...
    if flags not in (materialized_flags, passed_flags):
        errors.append("invalid C3 suite flags")
    ...
    oracle = json.loads((ROOT / "tests/fixtures/c3_review_calibration_v1/oracles.json").read_text(encoding="utf-8"))
    scenarios = json.loads((ROOT / "tests/integration/c3_review_scenarios.json").read_text(encoding="utf-8"))
    ...
    if fixture.get("synthetic_profile_id") != "c3_review_calibration_v1":
    ...
    test_path = ROOT / "tests/semantic/test_c3_review_contract.py"
    ...
    if errors:
        ...
        return 1
    print("PASSED: C3 Review & Calibration suite materialized and current business runner result is bound" if flags == passed_flags else "PASSED: C3 Review & Calibration suite materialized; no business test was executed")
    return 0

...
MANIFEST = ROOT / "tests/c2_suite_manifest.json"
REQUIRED = [f"C2-{index:03d}" for index in range(1, 11)]
...
def main() -> int:
    ...
    if flags not in (materialized_flags, passed_flags):
        errors.append("invalid C2 suite flags")
    ...
    oracle = json.loads((ROOT / "tests/fixtures/c2_hypothesis_v1/oracles.json").read_text(encoding="utf-8"))
    scenarios = json.loads((ROOT / "tests/integration/c2_hypothesis_scenarios.json").read_text(encoding="utf-8"))
    ...
    if fixture.get("synthetic_profile_id") != "c2_hypothesis_v1":
    ...
    test_path = ROOT / "tests/semantic/test_c2_hypothesis_contract.py"
    ...
    if errors:
        ...
        return 1
    print("PASSED: C2 Hypothesis Lifecycle suite materialized and current business runner result is bound" if flags == passed_flags else "PASSED: C2 Hypothesis Lifecycle suite materialized; no business test was executed")
    return 0

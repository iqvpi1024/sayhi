...
MANIFEST = ROOT / "tests/a4_suite_manifest.json"
REQUIRED = [f"A4-{index:03d}" for index in range(1, 9)]
...
def main() -> int:
    ...
    if flags not in (materialized_flags, passed_flags):
        errors.append("invalid A4 suite flags")
    ...
    oracle = json.loads((ROOT / "tests/fixtures/a4_access_policy_v1/oracles.json").read_text(encoding="utf-8"))
    scenarios = json.loads((ROOT / "tests/integration/a4_access_policy_scenarios.json").read_text(encoding="utf-8"))
    ...
    if fixture.get("synthetic_profile_id") != "a4_access_policy_v1":
    ...
    test_path = ROOT / "tests/semantic/test_a4_access_policy_contract.py"
    ...
    if errors:
        ...
        return 1
    print("PASSED: A4 Access Policy suite materialized and current business runner result is bound" if flags == passed_flags else "PASSED: A4 Access Policy suite materialized; no business test was executed")
    return 0

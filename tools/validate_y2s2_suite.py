...
REQUIRED = [f"Y2S2-{index:03d}" for index in range(1, 11)]
...
def main() -> int:
    ...
    if flags not in (materialized_flags, passed_flags):
        errors.append("invalid Y2S2 suite flags")
    ...
    oracle = json.loads((ROOT / "tests/fixtures/y2s2_local_model_v1/oracles.json").read_text(encoding="utf-8"))
    scenarios = json.loads((ROOT / "tests/integration/y2s2_local_model_scenarios.json").read_text(encoding="utf-8"))
    ...
    if fixture.get("synthetic_profile_id") != "y2s2_local_model_v1":
    ...
    test_path = ROOT / "tests/semantic/test_y2s2_local_model_contract.py"
    ...
    if errors:
        for error in errors:
            print(f"ERROR: {error}")
        return 1
    print("PASSED: Y2S2 suite materialized and current business runner result is bound" if flags == passed_flags else "PASSED: Y2S2 suite materialization preflight; no business test was executed")
    return 0

...
$expectedV06 = '4513B26860A334190AF8B8656A2A506D27224D78F88B567B37BB08DF423BCAD8'
if ((Get-Sha256Hex $v04) -ne $expectedV04) { Add-Error 'PRDv04 immutable canonical hash changed' }
if ((Get-Sha256Hex $v05) -ne $expectedV05) { Add-Error 'PRDv05 historical canonical hash changed' }
if ((Get-Sha256Hex $v06) -ne $expectedV06) { Add-Error 'PRDv06 approved canonical hash changed' }
if ($errors.Count -eq 0) { Add-Check 'PRD v0.4/v0.5 immutable and v0.6 approved hashes match' }

foreach ($line in @(
    'current_prd_path: PRDv06.md',
    'current_prd_version: 0.6',
    'current_prd_status: approved',
    "current_prd_canonical_lf_sha256: $expectedV06",
...
Add-Check 'Product baseline index points to PRD v0.6 and preserves v0.4/v0.5'
...
foreach ($line in @(
    ...
    '| 版本 | 0.6 |',
    '| 日期 | 2026-08-01 |'
)) { Assert-Contains $v06 $line "PRDv06 metadata missing: $line" }

$sections = @([regex]::Matches($v06, '(?m)^# ([0-9]+)\.') | ForEach-Object { [int]$_.Groups[1].Value })
if ($sections.Count -ne 27 -or (Compare-Object -ReferenceObject (1..27) -DifferenceObject $sections)) {
    Add-Error 'PRDv06 top-level sections are not exactly 1..27'
} else { Add-Check 'PRDv06 top-level sections are exactly 1..27' }

$v04Frs = @([regex]::Matches($v04, '\bFR-[0-9]{3}\b') | ForEach-Object { $_.Value } | Sort-Object -Unique)
$v06Frs = @([regex]::Matches($v06, '\bFR-[0-9]{3}\b') | ForEach-Object { $_.Value } | Sort-Object -Unique)
if ($v06Frs.Count -ne 32 -or (Compare-Object -ReferenceObject $v04Frs -DifferenceObject $v06Frs)) {
    Add-Error 'PRDv06 FR set differs from the 32-item v0.4 set'
} else { Add-Check 'PRDv06 preserves all 32 FR IDs without expansion' }

$objectSection = [regex]::Match($v06, '(?ms)^# 8\..*?(?=^## 8\.1)').Value
...
if ($objectRows.Count -ne 12 -or (Compare-Object -ReferenceObject $expectedObjects -DifferenceObject ($objectRows | Sort-Object -Unique))) {
    Add-Error 'PRDv06 core object table is not the approved 12-object set'
} else { Add-Check 'PRDv06 core object set is exactly 12' }

Assert-ClosedEnum $v06 'assertion_kind_values' @('observed','reported','quoted','opinion','inferred','analysis','predicted','fictional')
Assert-ClosedEnum $v06 'answer_status_values' @('verified','unconfirmed','disputed','not_covered','stale','unknown')
...
)) { Assert-Contains $v06 $requiredText "PRDv06 missing approved product boundary: $requiredText" }
...
foreach ($name in $stalePatterns.Keys) {
    if ([regex]::IsMatch($v06, $stalePatterns[$name])) { Add-Error "PRDv06 retains stale wording: $name" }
}
Add-Check 'Known v0.4 contradictory wording is absent from PRDv06'
...
foreach ($name in $privacyPatterns.Keys) {
    if ([regex]::IsMatch($v06, $privacyPatterns[$name])) { Add-Error "PRDv06 privacy heuristic matched: $name" }
}
Add-Check 'PRDv06 privacy heuristics found no configured pattern'

$fenceCount = [regex]::Matches($v06, '(?m)^```').Count
if (($fenceCount % 2) -ne 0) { Add-Error 'PRDv06 has unpaired Markdown fences' }
else { Add-Check 'PRDv06 Markdown fence parity is valid' }

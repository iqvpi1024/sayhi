...
MANIFEST = ROOT / "tests/c5_suite_manifest.json"
REQUIRED = [f"C5-{index:03d}" for index in range(1, 11)]
...
def main() -> int:
    ...
    if flags not in (materialized_flags, passed_flags):
        errors.append("invalid C5 suite flags")
    ...
    oracle = json.loads((ROOT / "tests/fixtures/c5_pack_backup_v1/oracles.json").read_text(encoding="utf-8"))
    scenarios = json.loads((ROOT / "tests/integration/c5_pack_scenarios.json").read_text(encoding="utf-8"))
    ...
    if fixture.get("synthetic_profile_id") != "c5_pack_backup_v1":
    ...
    test_path = ROOT / "tests/semantic/test_c5_pack_contract.py"
    ...
    if errors:
        ...
        return 1
    print("PASSED: C5 Context Pack & Encrypted Backup suite materialized and current business runner result is bound" if flags == passed_flags else "PASSED: C5 Context Pack & Encrypted Backup suite materialized; no business test was executed")
    return 0
